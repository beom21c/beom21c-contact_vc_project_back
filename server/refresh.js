    ...
    if (req.headers.authorization && req.headers.refresh) {
        ...
        if (authResult.ok === false && authResult.message === 'jwt expired') {
            ...
            if (result.ok === false) {
                ...
                db.query('select * from member where id = ? ', [decoded.id], async (err, results) => {
                    if (err) {
                    ...
                    const accessToken = jwts.sign({
                        id: id,
                        username: name
                    });
                    const refreshToken = jwts.refresh();
                    ...
                    res.status(200).send({ // client에게 토큰 모두를 반환합니다.
                        ...
                        userInfo : {company, name, id, position , email, phone}
                    });
                });
            }
        } else {
